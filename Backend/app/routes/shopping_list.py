...
async def crear_lista(body: ListCreate, current_user: User = Depends(get_current_user)):
    try:
        cast_ids = _cast_and_validate_ids(body.item_ids)
        doc = ShoppingList(name=body.name, items=cast_ids, user_id=current_user.id)
        await doc.insert()
...
async def listar_listas(current_user: User = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener listas: {str(e)}")


@router.get("/{list_id}", response_model=ListResponse)
async def obtener_lista(list_id: PydanticObjectId, current_user: User = Depends(get_current_user)):
    try:
        sl = await ShoppingList.find_one(ShoppingList.id == list_id, ShoppingList.user_id == current_user.id)
        if not sl:
...
async def actualizar_lista_endpoint(
    ...
    try:
        ...
        existing_list = await ShoppingList.find_one(ShoppingList.id == list_id, ShoppingList.user_id == current_user.id)
        ...
        raise HTTPException(status_code=500, detail=f"Error al actualizar lista: {str(e)}")


@router.delete("/{list_id}", status_code=status.HTTP_204_NO_CONTENT)
async def eliminar_lista_endpoint(list_id: PydanticObjectId, current_user: User = Depends(get_current_user)):
    try:
        # Verifica que la lista pertenezca al usuario
        existing_list = await ShoppingList.find_one(ShoppingList.id == list_id, ShoppingList.user_id == current_user.id)
        if not existing_list:
